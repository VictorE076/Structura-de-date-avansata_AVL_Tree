...
////
    ...
    Node* delete_(Node* init, const int val)
    {
        ...
        if (init == NULL)
            return init; // If the current deleted node had no children 
        init->height = 1 + max_unsigned_val_(height_(init->left), height_(init->right));
        int Bf = balance_factor_(init);
...
int main()
{
    ...
    */
    ...
    {
        t.delete_min_val();
        if(t.get_root_node() != NULL)
            cout << "\n\nRoot value: " << t.get_root_node()->value;
